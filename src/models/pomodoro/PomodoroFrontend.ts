import { DeletePomdoroBody } from "@/app/api/pomodoros/DELETE";
...
export default class PomodoroFrontend extends Pomodoro {
  ...
  public async deletePomodoro(): Promise<void> {
    this.validateId(this.id, "Delete pomodoro, invalid pomodoro id");
    const body: DeletePomdoroBody = {
      pomodoroId: this.id,
    };
    await axiosInstance.delete("/pomodoros", {
      data: body,
    });
  }
